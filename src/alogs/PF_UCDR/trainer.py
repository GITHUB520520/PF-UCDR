...
from src.models.PF_UCDR import PF_UCDR
import torch
import math
import time
...
from src.data.dataloaders import CuMixloader, BaselineDataset, BaselineEvalDataset
...
from src.sup_con_loss import soft_sup_con_loss, Euclidean_MSE, soft_sup_con_loss_patch, rgb_centroid_loss, \
    domain_confusion_loss, \
    triplet_loss_rgb, \
    triplet_loss_phase, triplet_loss_phase_rgb, triplet_loss_rgb_phase, domain_adversarial_loss
...
class Trainer:
    ...
    def __init__(self, args):
        ...
        # class dictionary
        self.dict_clss = utils.create_dict_texts(self.tr_classes)  # l : r
        ...
        print(tr_domains_unique)
        print(len(tr_domains_unique))
        ...
        self.model = PF_UCDR(self.args, self.dict_clss, self.dict_doms, device, isTest=False)
        ...
        # exit(0)
        path_log = os.path.join(args.root_path, 'src/alogs/PF_UCDR/logs', args.dataset, self.save_folder_name,
                                self.suffix)
    ...
    def training_set(self, stage):
        ...
        if stage == 1:
            ...
            train_parameters = ['text_prompt_learner.ctx', 'image_encoder.specific_domain_prompts',
                                'image_encoder.specific_class_prompts', 'stack_cross_attn_module',
                                'global_attn_module', 'fusing_vision_encoder.cross_attention_blocks']

        elif stage == 2:
            lr = self.args.lr
            lr = lr / 2
            print("========Training Target Prompt Generation phase========")
            train_parameters = ['image_encoder.layer_norm2', 'image_encoder.prompt_proj', 'image_encoder.W_image',
                                'image_encoder.W_prompt', 'image_encoder.feature_template',
                                'image_encoder.feature_proj', 'global_attn_module',
                                'stack_cross_attn_module', 'fusing_vision_encoder.cross_attention_blocks']
        ...
        if self.args.optimizer == 'sgd':
            ...
            self.optimizer = optim.SGD([
                ...
                {'params': adv_params, 'lr': lr * 0.1, 'name': 'domain_classifier'}
            ],
                weight_decay=self.args.l2_reg, momentum=self.args.momentum, nesterov=False,
                lr=lr)
    ...
    def do_epoch(self, stage, epoch):
        ...
        for i, (im, phase, cls, dom) in enumerate(train_loader):
            im = im.float().to(device, non_blocking=True)
            phase = phase.float().to(device, non_blocking=True)
            # 得到batch中每个类别的索引
            ...
            (feature, phase_feature, soft_label, cls_id, dom_id, queues, ph_queues,
             local_image_features, local_phase_features, cross_image_feature,
             combined_visual_feature, fused_features) = self.model(
                im, phase, dom,
                cls, stage)
            # print("cross_phase_feature is {}".format(cross_phase_feature))
            ...
            if epoch == 0:
                loss = (soft_sup_con_loss(feature, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss(phase_feature, soft_label, hard_labels, device=device) +
                        triplet_loss_rgb(feature, queues, cls_id, device=device) +
                        triplet_loss_phase(phase_feature, ph_queues, cls_id, device=device) +
                        triplet_loss_phase_rgb(phase_feature, queues, ph_queues, hard_labels) +
                        triplet_loss_rgb_phase(feature, queues, ph_queues, hard_labels) +
                        soft_sup_con_loss_patch(local_image_features, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss_patch(local_phase_features, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss(cross_image_feature, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss(combined_visual_feature, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss(fused_features, soft_label, hard_labels, device=device)
                        )
            else:
                loss = (soft_sup_con_loss(feature, soft_label, hard_labels, device=device) +
                        soft_sup_con_loss(phase_feature, soft_label, hard_labels, device=device) +
            ...
            if (i + 1) % self.args.log_interval == 0:
                print('[Train] Epoch: [{0}/{1}][{2}/{3}]\t'
                      ...
                      .format(self.current_epoch + 1, self.args.epochs, i + 1, len(train_loader), batch_time=batch_time,
                              net=total_loss))
    ...
    def do_training(self):
        ...
        for self.current_epoch in range(self.start_epoch, self.args.epochs):
            ...
            start = time.time()
            # self.model.update_grl_alpha(self.current_epoch, self.args.epochs)
            ...
            if self.args.dataset == 'DomainNet':
                if self.args.ucddr == 0:
                    ...
                    for domain in [self.args.seen_domain, self.args.holdout_domain]:
                        for includeSeenClassinTestGallery in [0, 1]:
                            ...
                            splits_gallery = domainnet.trvalte_per_domain(self.args, self.args.gallery_domain,
                                                                          includeSeenClassinTestGallery,
                                                                          self.tr_classes, self.va_classes,
                                                                          self.te_classes)

                            data_te_query = BaselineEvalDataset(np.array(splits_query['te']),
                                                                transforms=self.image_transforms['eval'])
                            data_te_gallery = BaselineEvalDataset(np.array(splits_gallery['te']),
                                                                  transforms=self.image_transforms['eval'])
                            ...
                            te_loader_gallery = DataLoader(dataset=data_te_gallery,
                                                           batch_size=self.args.batch_size * 10, shuffle=False,
                                                           num_workers=self.args.num_workers, pin_memory=True)
                    ...
                    if self.args.holdout_domain == 'quickdraw':
                        ...
                        p = 0.25
                    # udcdr
                    splits_query = domainnet.seen_cls_te_samples(self.args, self.args.holdout_domain, self.tr_classes,
                                                                 p)
                    splits_gallery = domainnet.seen_cls_te_samples(self.args, self.args.gallery_domain, self.tr_classes,
                                                                   p)
                    data_te_query = BaselineEvalDataset(np.array(splits_query),
                                                        transforms=self.image_transforms['eval'])
                    data_te_gallery = BaselineEvalDataset(np.array(splits_gallery),
                                                          transforms=self.image_transforms['eval'])
                    # PyTorch test loader for query
                ...
                data_te_query = BaselineEvalDataset(self.data_splits['query_te'],
                                                    transforms=self.image_transforms['eval'])
                data_te_gallery = BaselineEvalDataset(self.data_splits['gallery_te'],
                                                      transforms=self.image_transforms['eval'])
            ...
            print(
                f"Epoch Time:{elapsed // 60:.0f}m{elapsed % 60:.0f}s lr:{utils.get_lr(self.optimizer):.7f} mAP:{map_:.4f} prec:{prec:.4f}\n")
            ...
            if map_ > self.best_map:
                ...
                model_save_name = 'val_map-' + '{0:.4f}'.format(map_) + '_prec-' + '{0:.4f}'.format(
                    prec) + '_ep-' + str(self.current_epoch + 1) + self.suffix
    ...
    def adjust_learning_rate(self, min_lr=1e-6):
        base_lr = self.args.lr * math.pow(1e-3, float(self.current_epoch) / 20)
        base_lr = max(base_lr, min_lr)

        if len(self.optimizer.param_groups) >= 2:
            self.optimizer.param_groups[0]['lr'] = base_lr
            self.optimizer.param_groups[1]['lr'] = base_lr * 0.1
        else:
            self.optimizer.param_groups[0]['lr'] = base_lr
...
@torch.no_grad()
def evaluate(loader_sketch, loader_image, model: PF_UCDR, dict_clss, dict_doms, stage, args):
    # Switch to test mode
    model.eval()
