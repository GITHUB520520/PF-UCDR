...
def trvalte_per_domain(datapath, is_eccv_split=True):
    ...
    np.random.seed(0)
    trval_classes = np.setdiff1d(classes, te_classes)
    tr_classes = np.random.choice(trval_classes, int(0.9*len(trval_classes)), replace=False)
    va_classes = np.setdiff1d(trval_classes, tr_classes)
...
root_path = '/app/data/'
path_im = os.path.join(root_path, 'Sketchy', 'extended_photo')
tr_classes, va_classes, te_classes, splits_im = trvalte_per_domain(path_im, is_eccv_split)
...
for cname in tr_classes.tolist() + va_classes.tolist():
	if cname not in wnid_dict.keys():
...
# to_save = os.path.join('cid_mask_eccv_split.pkl')
